...
export async function GET(request: NextRequest) {
  try {
    console.log('테이블 목록 조회 프록시 요청');
    
    // 백엔드 API로 프록시
    const response = await fetch(`${API_BASE_URL}/sql-execute/tables`, {
      method: 'GET',
      headers: {
    ...
    console.log('프록시 응답 성공:', Array.isArray(data) ? `${data.length}개 테이블` : '단일 객체');
    
    return NextResponse.json(data);
    
  } catch (error) {
    console.error('테이블 목록 프록시 오류:', error);
